...
internal static class LocationsHelpers
{
    ...
    internal static async Task<List<LocationDto>?> GetLocations(this HttpClient webClient)
    {
        try
        {
    ...
    internal static async Task<LocationDto> AssertCreateLocationFunctions(this HttpResponseMessage httpResponse, LocationDto request, HttpClient webClient)
    {
        ...
        // Note: The original test expected http://localhost/api/hotels/{id}. AspNetCore's CreatedAtAction might produce absolute URI.
        // We will match the pattern generically or exact if we trust the environment.
        ...
        // Let's use string checks to be safer if port varies, but usually test server is standard.
        // The original code was strict.
        httpResponse.Headers.Location.Should().Be($"http://localhost/api/locations/{resultDto.Id}", "we expect the location header to point to the get location by id endpoint");
    ...
    internal static async Task<List<LocationDto>> AssertLocationListAllFunctions(this HttpResponseMessage httpResponse)
    {
        ...
        // Verify TableCount is present and reasonable (though existing data might vary, we seeded > 0)
        resultDto.All(x => x.TableCount >= 1).Should().BeTrue("we expect all locations to have at least 1 table");
